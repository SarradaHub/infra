...
module "kafka" {
  ...
  cluster_name           = "platform-events-dev"
  kafka_version          = "3.7.0"
...
module "schema_registry" {
  ...
  registry_name = "platform-events-dev"
  tags          = local.tags
  schemas = {
    "user-created" = {
      description = "User created canonical event"
      schema_def  = file("../../../../platform/contracts/schemas/identity/user.created.v1.json")
    }
    "match-scheduled" = {
      description = "Match scheduled event"
      schema_def  = file("../../../../platform/contracts/schemas/scheduling/match.scheduled.v1.json")
    }
    "bet-market-created" = {
      description = "Bet market created event"
      schema_def  = file("../../../../platform/contracts/schemas/betting/bet.market.created.v1.json")
    }
    "wager-accepted" = {
      description = "Wager accepted event"
      schema_def  = file("../../../../platform/contracts/schemas/betting/wager.accepted.v1.json")
    }
    "payment-received" = {
      description = "Payment received event"
      schema_def  = file("../../../../platform/contracts/schemas/finance/payment.received.v1.json")
    }
    "ledger-transaction-recorded" = {
      description = "Ledger transaction recorded event"
      schema_def  = file("../../../../platform/contracts/schemas/ledger/ledger.transaction.recorded.v1.json")
    }
  }
}
...
module "observability" {
  ...
  grafana_workspace_name = "platform-grafana-dev"
  amp_workspace_alias    = "platform-events-dev"
  tags                   = local.tags
  metric_alarms = [
    {
      name                = "dev-kafka-active-controller"
      ...
      alarm_description   = "Kafka controller count dropped below 1"
    }
  ]
}

...
module "kafka" {
  ...
  cluster_name           = "platform-events-prod"
  kafka_version          = "3.7.0"
...
module "schema_registry" {
  ...
  registry_name = "platform-events-prod"
...
module "observability" {
  ...
  grafana_workspace_name = "platform-grafana-prod"
  amp_workspace_alias    = "platform-events-prod"
  tags                   = local.tags
  metric_alarms = [
    {
      name                = "prod-kafka-active-controller"
      ...
      alarm_description   = "Kafka controller count dropped below 1 (prod)"
    }
  ]
}
...
module "render_service" {
  ...
  plan           = lookup(each.value, "plan", "standard")
